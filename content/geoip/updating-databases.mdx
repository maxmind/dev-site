...
## Significant changes in database size

It is difficult for us to be able to provide the exact file size difference
between GeoIP database updates. This is because [we now update some of our GeoIP
databases twice weekly in order to more quickly incorporate data corrections and
updates](/geoip/release-notes?lang=en#twice-weekly-updates-for-some-geoip2-databases).
Sizable swings in file size are possible as we receive and consolidate new data.
Large IPv6 range activations is one specific example when these swings can
occur. We understand that the unpredictability may impact your work, and we
apologize for the inconvenience.

We plan to post in our [GeoIP release notes](/geoip/release-notes) when we
forecast a sizable increase in file size, and if this may cause problems for you
we recommend delaying your deployment of the next release until the updated file
is available and the exact file size is known. We may be able to provide more
detailed projections in file size changes in the future.
