...
endpoints to check for the validity of a license key would fail as license keys
are bound to certain products only.

You could use this API for building tools to scan your code for secrets to
ensure that secrets are not published in improper locations. One example of such
a tool is trufflehog. Using such a tool, you can prevent license keys
accidentally ending up in unexpected places and enhance the security of your
applications.
