...
{{< release-note date="2025-02-27" title="Off schedule release for GeoIP and GeoLite location databases" >}}

On Thursday, February 27, we will be issuing an off-schedule release of the following databases:
...
We are correcting country-level geolocation data for a number of networks
associated with the Sky Broadband ISP. Additional minor data corrections will be
included in this release as well.

As part of this release, we are also releasing an updated GeoIP ISP database,
but there are no concerns with the previous GeoIP ISP database. The GeoIP ISP
database released today includes regular updates and minor corrections
consistent with a normal release.
