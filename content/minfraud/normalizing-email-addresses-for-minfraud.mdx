...
Our [API clients](#todo) do this for you if you enable sending the MD5 hash.
...
`@`).
1. Trim whitespace from both ends of the domain.
1. If the domain ends with a period, trim it off.
...
1. Remove alias parts from the local part. For `yahoo.com` addresses, this is
everything after and including the first `-` character, if present. For
`gmail.com` addresses, this is everything after and including the first `+`
