---
draft: false
title: Updating GeoIP and GeoIP Legacy Databases
---
...
We highly recommend using the GeoIP Update program, but there may be some
instances where you are unable to use the program or if you are using
CSV-format databases.

## Using GeoIP Update

MaxMind provides the GeoIP Update program, which performs automatic updates for
both GeoIP2 and GeoIP Legacy binary databases.
...
[here](https://github.com/maxmind/geoipupdate) for installation
instructions. It can also be installed via our [Docker
...
**For GeoIP2 and GeoIP Legacy Databases**

Log in to your account portal to download a partially [pre-filled configuration
file](https://www.maxmind.com/en/accounts/current/license-key/GeoIP.conf) and save it in your configuration directory (e.g.,
`/usr/local/etc/`) as `GeoIP.conf`. You will need to replace the
`YOUR_LICENSE_KEY_HERE` placeholder with an active license key associated with
your MaxMind account. You can see your license key information on [your account
License Keys page](https://www.maxmind.com/en/accounts/current/license-key).

You may also write this file by hand using the template below (**not
recommended**).
...
7 9 * * 3 /usr/local/bin/geoipupdate
# end of crontab
```
...
request for that database’s download permalink URL. The download permalink can be found in the [Download 
Databases](https://www.maxmind.com/en/accounts/current/geoip/downloads)  section of your account portal.
...
downloads](https://support.maxmind.com/geolite-faq/general/is-there-a-limit-to-how-often-i-can-download-a-database-from-my-maxmind-account/) made within a limited period of time.*
