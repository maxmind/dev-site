...
number of files. In this tutorial will be working with the following files:
...
In this case, you are over-writing the CSV files you downloaded, with the first
and last IP addresses from the `network` field converted from CIDR notation to a
hexidecimal number.
...
that MySQL will be able to utilise the indexes we've created more efficiently:
