...
| **minfraud_id**     | string     | _Conditionally required.[^1]_ A UUID that identifies a minFraud Score, minFraud Insights, or minFraud Factors request. This ID is returned at `/id` in the response.                                                                                                  |
...
| ----------------------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| JSON_INVALID            | 400 Bad Request           | The request body is not a single JSON object.                                                                                                                                                                                         |
| MAXMIND_ID_INVALID      | 400 Bad Request           | You have supplied an invalid `maxmind_id`. Check your `maxmind_id` to ensure that it is 8 characters in length and made up only of digits and letters. The value is case sensitive. MaxMind issues `maxmind_id` values in upper case. |
...
| `<FIELD>`\_INVALID      | 400 Bad Request           | A field has the wrong JSON type, for example `TRANSACTION_ID_INVALID`. `TRANSACTION_ID_INVALID` and `CHARGEBACK_CODE_INVALID` are also returned when the field contains a NUL character. `<FIELD>` is the field name in upper case.   |
